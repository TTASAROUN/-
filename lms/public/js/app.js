...
const rawVal = (f, r) => { const v = f.calc ? f.calc(r) : r[f.k]; if (f.t === 'ref') return refLabel(f.r, v); if (f.t === 'bool') return v ? '예' : '아니오'; if (f.t === 'file') return v && v.name || ''; if (Array.isArray(v)) return v.join(' / '); if (v && typeof v === 'object') return JSON.stringify(v); return v ?? ''; };
...
function renderShell() {
  ...
  root.innerHTML = `<div class="app">
    <aside class="side" id="side">
      <div class="brand"><div class="sub">LMS 업무 콘솔</div><div class="org">${esc(org.name)}</div></div>
      ...
      <div class="foot"><span>${esc(S.user.name)} · ${esc(S.user.role)}</span><button class="btn sm ghost" id="logout">로그아웃</button></div>
    </aside>
    <main class="main">
...
function renderList(page, col, filterCourse) {
  ...
  page.innerHTML = `<div class="toolbar no-print">
      ...
      ${hasCourse ? `<select id="courseF"><option value="">전체 과정</option>${S.list('course').map(c => `<option value="${c.id}" ${c.id === courseF ? 'selected' : ''}>${esc(c.name)}</option>`).join('')}</select>` : ''}
  ...
  if (col === 'training_log') $('#genLog').onclick = async () => {
    const tt = S.list('timetable').filter(t => t.date === today() && (!courseF || t.course === courseF));
    if (!tt.length) return toast('오늘 날짜의 시간표가 없습니다. 2. 과정등록 > 시간표 등록에서 먼저 등록하세요.', true);
    let n = 0; for (const t of tt) { if (S.list('training_log').some(l => l.date === t.date && l.course === t.course && l.period === t.period)) continue; await S.add('training_log', { course: t.course, date: t.date, period: t.period, subject: t.subject, teacher: t.teacher, approval: '작성' }); n++; }
...
const staffIdOfUser = () => { const s = S.list('staff').find(s => s.name === (S.user || {}).name); return s ? s.id : ''; };
...
async function importCSV(col, file) {
  ...
  for (const l of lines.slice(1)) {
    const v = parse(l), rec = {};
    ent.fields.forEach(f => { const i = head.findIndex(h => h === f.l || h === f.k || (f.k === 'subject' && /과목|능력단위|훈련내용/.test(h)) || (f.k === 'date' && /일자|날짜|훈련일/.test(h)) || (f.k === 'period' && /교시/.test(h)) || (f.k === 'start' && /시작/.test(h)) || (f.k === 'end' && /종료/.test(h)) || (f.k === 'room' && /강의실|장소/.test(h))); if (i < 0) return; let x = v[i]; if (f.t === 'ref') { const m = S.list(f.r).find(r => r[REF_LABEL[f.r] || 'name'] === x); x = m ? m.id : undefined; } if (f.t === 'number') x = +x || 0; if (x !== undefined && x !== '') rec[f.k] = x; });
...
function openForm(col, rec = {}, onDone) {
  ...
  ent.fields.forEach(f => { if (rec[f.k] === undefined && f.def !== undefined) rec[f.k] = f.def; });
  ...
  $('#fSave', bg).onclick = async () => {
    ...
    if (isNew) for (const f of ent.fields) if (f.auto === 'certNo') out[f.k] = nextCertNo();
...
function nextCertNo() { const org = S.settings(); const y = new Date().getFullYear(); const prefix = `${org.code || 'ORG'}-${y}-`; const n = S.list('certificate').filter(c => (c.number || '').startsWith(prefix)).length + 1; return prefix + String(n).padStart(3, '0'); }
function fieldHTML(f, rec) {
  ...
  else if (f.t === 'ref') inp = `<select id="${id}"><option value="">선택</option>${S.list(f.r).map(r => `<option value="${r.id}" ${r.id === v ? 'selected' : ''}>${esc(r[REF_LABEL[f.r] || 'name'])}${f.r === 'trainee' && r.course ? ` (${esc(refLabel('course', r.course))})` : ''}</option>`).join('')}</select>`;
...
const courseSelect = (id, cur, all = '전체 과정') => `<select id="${id}"><option value="">${all}</option>${S.list('course').map(c => `<option value="${c.id}" ${c.id === cur ? 'selected' : ''}>${esc(c.name)}</option>`).join('')}</select>`;
...
VIEWS.dashboard = (page) => {
  const tr = S.list('trainee'), active = tr.filter(t => t.status === '재학'), courses = S.list('course');
  const t0 = today(); const d30 = new Date(); d30.setDate(d30.getDate() - 30); const s30 = d30.toISOString().slice(0, 10);
  const done = tr.filter(t => ['수료', '취업'].includes(t.status)), emp = S.list('employment');
  const empRate = done.length ? Math.round(done.filter(t => emp.some(e => e.trainee === t.id)).length / done.length * 100) : 0;
  const alerts = [];
  const ttToday = S.list('timetable').filter(t => t.date === t0), logs = S.list('training_log');
  ...
  const pendM = S.list('makeup_log').filter(l => l.approval !== '승인'); if (pendM.length) alerts.push(['warn', `결재 대기 보강일지 ${pendM.length}건`, '', '#/entity/makeup_log']);
  const noCert = done.filter(t => !S.list('certificate').some(c => c.trainee === t.id)); if (noCert.length) alerts.push(['info', `증서 미발급 수료자 ${noCert.length}명`, noCert.slice(0, 5).map(t => t.name).join(', ') + (noCert.length > 5 ? ' 외' : ''), '#/entity/certificate']);
  const cs = S.list('trainee_counsel'); const noCounsel = active.filter(t => !cs.some(c => c.trainee === t.id && c.date >= s30)); if (noCounsel.length) alerts.push(['warn', `최근 30일 상담 없는 재학생 ${noCounsel.length}명`, noCounsel.slice(0, 6).map(t => t.name).join(', ') + (noCounsel.length > 6 ? ' 외' : ''), '#/entity/trainee_counsel']);
  const gr = S.list('grievance').filter(g => g.status !== '완료'); if (gr.length) alerts.push(['warn', `미처리 고충/건의 ${gr.length}건`, '', '#/entity/grievance']);
  const sv = S.list('survey').filter(s => s.status === '진행중' && s.end && s.end <= t0); if (sv.length) alerts.push(['info', `마감일 지난 설문 ${sv.length}건`, '결과분석 후 마감 처리하세요', '#/entity/survey']);
  ...
  const nextCare = S.list('aftercare').filter(a => a.next && a.next <= t0); if (nextCare.length) alerts.push(['warn', `사후관리 예정일 도래 ${nextCare.length}건`, '', '#/entity/aftercare']);
  if (!alerts.length) alerts.push(['good', '오늘 처리할 누락 업무가 없습니다', '', '']);
  const consultToday = S.list('admission_consult').filter(a => a.date === t0).length;
  const sched = S.list('schedule').filter(s => s.date >= t0).sort((a, b) => a.date > b.date ? 1 : -1).slice(0, 6);
  page.innerHTML = `<div class="kpis">
      ...
      <div class="kpi"><div class="l">오늘 입학상담</div><div class="v">${consultToday}</div><div class="s">상담중 ${S.list('admission_consult').filter(a => a.status === '상담중').length}건</div></div>
...
VIEWS.enrollment = (page) => {
  const tr = S.list('trainee'); const byC = groupBy(tr, t => t.course);
  page.innerHTML = `<div class="toolbar no-print"><span class="grow"></span>${printBtn}<a class="btn primary" href="#/entity/trainee">훈련생 등록/수정 →</a></div>
    <div class="card"><h2>과정별 등록 현황</h2>${table(['과정', '정원', '등록', '재학', '수료', '취업', '중도탈락', '충원율%'], S.list('course').map(c => { const l = byC[c.id] || []; const n = (s) => l.filter(t => t.status === s).length; return [esc(c.name), c.capacity || '-', l.length, n('재학'), n('수료'), n('취업'), n('중도탈락'), c.capacity ? Math.round(l.length / c.capacity * 100) : '-']; }))}</div>
...
VIEWS.interview_result = (page) => {
  const list = S.list('interview_eval'); const f = E.interview_eval.fields;
...
function counselView(page, mode) {
  const list = S.list('trainee_counsel');
...
VIEWS.pre_eval_analysis = (page) => {
  const infos = S.list('pre_eval_info');
  ...
  const draw = () => { const ev = $('#ev').value; const rows = S.list('pre_eval').filter(r => r.evalInfo === ev); const f = E.pre_eval.fields; const tot = rows.map(r => total(f, r));
...
VIEWS.grade_report = (page, [traineeId]) => {
  const tr = S.list('trainee'); const gf = E.grade.fields;
  ...
  const draw = () => { const cf = $('#cf').value, tid = $('#tr').value; const org = S.settings();
    const settings = S.list('subject_eval_setting').filter(s => !cf || s.course === cf); const grades = S.list('grade').filter(g => (!cf || g.course === cf));
    if (tid) { const t = S.get('trainee', tid); const gs = grades.filter(g => g.trainee === tid); const tot = gs.map(g => total(gf, g));
      $('#out').innerHTML = `<div class="card"><div style="text-align:center;margin-bottom:16px"><h2 style="font-size:22px">개별 성적표</h2><div class="muted">${esc(org.name)} · ${esc(refLabel('course', t.course))}</div></div>
...
VIEWS.unit_analysis = (page) => {
  ...
  const draw = () => { const cf = $('#cf').value; const gf = E.grade.fields; const settings = S.list('subject_eval_setting').filter(s => !cf || s.course === cf);
    const rows = settings.map(s => { const gs = S.list('grade').filter(g => g.setting === s.id); const tot = gs.map(g => total(gf, g)); return { s, gs, tot }; });
...
VIEWS.deviation = (page) => {
  page.innerHTML = `<div class="toolbar no-print">${courseSelect('cf')}<span class="grow"></span>${printBtn}</div><div id="out"></div>`;
  const draw = () => { const cf = $('#cf').value; const gf = E.grade.fields; const grades = S.list('grade').filter(g => !cf || g.course === cf); const settings = S.list('subject_eval_setting').filter(s => !cf || s.course === cf);
    const all = grades.map(g => total(gf, g)); const m = avg(all), s = sd(all);
    const trs = S.list('trainee').filter(t => !cf || t.course === cf).map(t => { const gs = grades.filter(g => g.trainee === t.id); const a = gs.length ? avg(gs.map(g => total(gf, g))) : null; return { t, a, gs }; }).filter(x => x.a != null).sort((x, y) => y.a - x.a);
...
VIEWS.survey_analysis = (page, [sid]) => {
  ...
  const draw = () => { const s = S.get('survey', $('#sv').value); if (!s) return $('#out').innerHTML = '<div class="card empty">설문을 먼저 만드세요.</div>';
    const rs = S.list('survey_response').filter(r => r.survey === s.id);
...
VIEWS.cert_print = (page, [cid]) => {
  const certs = S.list('certificate');
  ...
  const draw = () => { const c = S.get('certificate', $('#ct').value); if (!c) return $('#out').innerHTML = '<div class="card empty">발급된 증서가 없습니다. 증서번호발급에서 먼저 발급하세요.</div>';
    const t = S.get('trainee', c.trainee) || {}, co = S.get('course', c.course) || {}, org = S.settings();
...
VIEWS.cert_ledger = (page) => {
  const certs = S.list('certificate').slice().sort((a, b) => (a.number || '') > (b.number || '') ? 1 : -1);
  page.innerHTML = `<div class="toolbar no-print"><span class="grow"></span>${printBtn}<button class="btn" id="csv">CSV 내려받기</button></div>
    <div class="card"><h2>증서 발급대장 <span class="muted small">${esc(S.settings().name)} · 총 ${certs.length}건 · 자동 생성</span></h2>${table(['증서번호', '종류', '성명', '생년월일', '과정', '발급일', '발급자', '비고'], certs.map(c => { const t = S.get('trainee', c.trainee) || {}; return [`<span class="mono">${esc(c.number)}</span>`, badge(c.kind), esc(t.name), esc(t.birth || ''), esc(refLabel('course', c.course)), c.date, esc(refLabel('staff', c.issuer)), esc(c.memo || '')]; }))}</div>`;
...
VIEWS.calendar = (page) => {
  ...
  const draw = () => { const y = cur.getFullYear(), m = cur.getMonth(); $('#ym').textContent = `${y}년 ${m + 1}월`;
    ...
    const ev = (d) => [...S.list('schedule').filter(s => s.date === d).map(s => ({ k: s.kind, t: (s.time ? s.time + ' ' : '') + s.title })), ...S.list('course').filter(c => c.start === d).map(c => ({ k: '과정', t: '개강: ' + c.name })), ...S.list('course').filter(c => c.deadline === d).map(c => ({ k: '과정', t: '모집마감: ' + c.name })), ...S.list('subject_eval_setting').filter(s => s.date === d).map(s => ({ k: '평가', t: '평가: ' + s.unit })), ...S.list('survey').filter(s => s.end === d).map(s => ({ k: '기타', t: '설문마감: ' + s.title }))];
...
VIEWS.stats = (page) => {
  ...
  const draw = () => { const cf = $('#cf').value, stt = $('#st').value; const tr = S.list('trainee').filter(t => (!cf || t.course === cf) && (!stt || t.status === stt)); const ac = S.list('admission_consult').filter(a => !cf || a.course === cf);
...
VIEWS.settings = (page) => {
  ...
  page.innerHTML = `<div class="card"><h2>기관 정보 <span class="muted small">홈페이지와 증서에 그대로 표시됩니다</span></h2><div class="form">${F.map(([k, l]) => `<div class="field ${k === 'address' || k === 'slogan' ? 'wide' : ''}"><label for="s_${k}">${l}</label><input id="s_${k}" type="text" value="${esc(org[k] || '')}"></div>`).join('')}
    <div class="field wide"><label for="s_about">기관 소개 (홈페이지)</label><textarea id="s_about">${esc(org.about || '')}</textarea></div></div>
    <div style="margin-top:14px"><button class="btn primary" id="save">저장</button></div></div>
    <div class="card"><h2>데이터 관리</h2><div class="toolbar"><button class="btn" id="exp">전체 백업 내려받기 (JSON)</button><button class="btn" id="imp">백업 불러오기</button><input type="file" id="impF" accept=".json" hidden><span class="grow"></span>${org.demo ? `<button class="btn danger" id="reset">예시 데이터 전체 삭제 (실제 사용 시작)</button>` : ''}</div>
  ...
  $('#save').onclick = async () => { const o = { ...org, id: 'main' }; F.forEach(([k]) => o[k] = $('#s_' + k).value.trim()); o.about = $('#s_about').value; await S.update('settings', 'main', o); toast('저장했습니다'); renderShell(); route(); };
  ...
  if (org.demo) $('#reset').onclick = async () => { if (!confirm('예시 데이터를 모두 지우고 빈 상태로 시작합니다. 되돌릴 수 없습니다. 계속할까요?')) return; const keep = { ...org, demo: false }; await S.resetDemo(); await S.update('settings', 'main', keep); toast('예시 데이터를 지웠습니다'); renderShell(); location.hash = '#/view/dashboard'; route(); };
};
